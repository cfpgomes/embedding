...
ax1.hist([set1_samples_energy, set2_samples_energy], bins='fd', color=['red', 'blue'], orientation='horizontal', rwidth=1, label=['normal', 'clique'])

ax2.hist([set1_samples_value, set2_samples_value], bins='fd', color=['red', 'blue'], orientation='horizontal', rwidth=1, label=['normal', 'clique'])


# Tidy up the figure
ax1.grid(True)
ax1.legend()
ax1.set_ylabel('Solution Energy')
...
folder_name = "histograms_energy_vs_objective_value"

# Check if folder exists and creates if not
if not os.path.exists('images/' + folder_name):
    os.makedirs('images/' + folder_name)

fig.text(0.5, 0.005, 'How to interpret: The lower the value, the better. Hence, the bigger the share of solutions in the lowest bins, the better.',
         ha='center', size='xx-small')

output_name = f'N{N}B{B}q{q}P{P}W{work_id}{date}'
fig.suptitle('Histograms Energy and Objective Value - ' + output_name)
